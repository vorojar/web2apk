...
- 📬 **FCM 推送** - Firebase 远程推送（海外，需上传 google-services.json）
- 📱 **设备信息** - 获取设备型号、系统版本、设备唯一ID
- 📞 **电话/短信** - 调用系统拨号/短信界面
- 📷 **JS 相机** - 非表单拍照，直接返回 Base64 图片数据
- 🖼️ **图片预览** - 原生图片查看器，支持缩放、保存、分享
- 🎤 **录音功能** - 录制音频，返回 Base64 数据（语音消息场景）
- 📹 **视频录制** - 录制视频，支持前后摄像头、时长限制、画质选择
- 🔒 **防截屏** - 禁止截屏/录屏，保护隐私页面（支付、聊天等）
- 📲 **APP 唤端** - 打开淘宝/小红书/地图等 APP，支持检测安装状态
- 🔄 **应用内更新** - 获取版本、下载 APK、安装更新（网页控制更新流程）
- 🔤 **字体缩放** - 跟随系统字体大小，支持 JS 动态调整

### 性能优化
- ⚡ **WebView 预热** - 启动时预加载 WebView 引擎，加快首屏速度
...
### v1.8.0
- 新增：启动耗时统计 - 精确获取各阶段耗时，帮助优化启动性能
- 新增：App 状态监听 - 前后台状态切换感知
- 新增：Toast 提示 - 显示原生 Toast 提示
- 新增：电量信息 - 获取电量和充电状态
- 新增：陀螺仪/加速计 - 获取传感器数据
...
- 新增：后台音频播放 - 配合前台服务实现音频后台播放
- 新增：图片预览 - 原生图片查看器，支持缩放、保存、分享
- 新增：录音功能 - 网页可调用录音，返回 Base64 音频数据（语音消息场景）
- 新增：视频录制 - 支持前后摄像头切换、时长限制、画质选择（low/medium/high）
- 新增：防截屏 - 网页可开启/关闭防截屏模式，保护隐私页面
- 新增：APP 唤端 - 打开淘宝/小红书/地图等 APP，支持检测安装、备用网页跳转

### v1.5.1
- 修复：离线错误页按返回键进入白屏的问题（现在直接退出应用）
- 修复：离线错误页点击"重新加载"出现白屏的问题
- 优化：重试按钮增加网络检测，无网络时提示用户而非加载白屏
- 优化：VPN 网络判断逻辑，单独开 VPN 但无底层网络时正确识别为无网络
